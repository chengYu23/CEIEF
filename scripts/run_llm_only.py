"""
CEIEF LLM-Only Experiment Runner
运行 LLM-Only 基线条件实验（无KG，无Agent）

用法：python scripts/run_llm_only.py --task_id T001 --turns 5
"""
...
from src.dialogue_engine import DialogueEngine, CONDITION_LLM_ONLY
...
def run_session(task_id: str, max_turns: int = 5, config_path: str = "config.yaml"):
    ...
    session_id = engine.start_session(
        condition=CONDITION_LLM_ONLY,
        task_family=task["family"],
        role_profile=task["role_profile"]
    )
    logger.info(f"LLM-Only session: {session_id} | task={task_id}")
    ...
    engine.close()
    print(f"\nLLM-Only session done. Turns: {session.total_turns}")
